...
HIGH_RISK_KEYWORDS = [
    "credit score", "loan decision", "employment", "hiring", "termination",
    "medical diagnosis", "treatment recommendation", "law enforcement",
    "biometric", "facial recognition", "emotion recognition", "critical infrastructure",
    "educational assessment", "border control", "asylum", "benefits eligibility"
]

PROHIBITED_KEYWORDS = [
    "social scoring", "mass surveillance", "subliminal manipulation",
    "exploit vulnerabilities", "real-time biometric public spaces"
]
...
class RiskClassifier:
    ...
    def classify(prompt: str, response: str, context: dict = None) -> Dict:
        ...
        if any(kw in text for kw in ["customer service", "chatbot", "virtual assistant", "recommend"]):
            return {
                "level": "limited",
                "score": 0.35,
...
class ComplianceFlagger:
    ...
    def check(prompt: str, response: str, pii_types: List[str]) -> List[Dict]:
        ...
        if any(kw in response.lower() for kw in ["as an ai", "i cannot", "i'm not able to"]):
            flags.append({
                "code": "AI_DISCLOSURE",
                "severity": "info",
...
class AgentInterceptorMiddleware(BaseHTTPMiddleware):
    ...
    async def _log_interaction(self, request: Request, body: dict,
                                prompt: str, response: str, response_data: dict):
        """Log the interaction to the audit database."""
        try:
            # PII detection
            pii_found, pii_types, pii_risk = self.pii_detector.scan(f"{prompt} {response}")
            ...
            for choice in response_data.get("choices", []):
                ...
                tool_calls.extend([{"name": t.get("function", {}).get("name"), "id": t.get("id")} for t in tc])
            ...
            log_entry = {
                ...
                "metadata": json.dumps({"risk_level": risk_info["level"], "eu_article": risk_info["eu_ai_act_article"]}),
            ...
            if risk_score > 0.6:
                logger.warning(f"HIGH RISK interaction detected: score={risk_score:.2f}, flags={len(flags)}")
